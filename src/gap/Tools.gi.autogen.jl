...
@InstallMethod( DefiningTripleOfUnderlyingQuiver,
  ...
  function( cat )
    ...
    return Triple( Length( objs ),
                   Length( mors ),
                   List( mors, mor ->
                         PairGAP( -1 + SafeUniquePositionProperty( objs, obj -> IsEqualForObjects( cat, obj, Source( mor ) ) ),
...
@InstallMethod( Size,
  ...
  function( C )
    ...
    if (!( HasIsFiniteCategory( C ) && IsFiniteCategory( C ) && HasRangeCategoryOfHomomorphismStructure( C ) ))
        TryNextMethod( );
    end;
...
CAP_INTERNAL_ADD_REPLACEMENTS_FOR_METHOD_RECORD(
        @rec(
            ...
            [ [ "HomomorphismStructureOnObjects", 2 ], ## called in List
              [ "IdentityMorphism", 3 ], ## called alone && in List
...
@InstallMethod( YonedaEmbeddingData,
  ...
  function( C )
    ...
    @Assert( 0, HasDefiningTripleOfUnderlyingQuiver( C ) );
    
    #% CAP_JIT_DROP_NEXT_STATEMENT
    @Assert( 0, HasRangeCategoryOfHomomorphismStructure( C ) );
    ...
    Yoneda_on_objs =
      function ( obj )
        ...
        return PairGAP( Yobj_on_objs, Yobj_on_mors );
...
@BindGlobal( "NERVE_TRUNCATED_IN_DEGREE2_DATA",
  function ( B )
    ...
    #% CAP_JIT_DROP_NEXT_STATEMENT
    @Assert( 0, ForAll( mus, mu -> ForAll( mu, m -> IsWellDefined( m[1] ) && IsWellDefined( m[2] ) ) ) );
                        ...
                        AsList(
                               PreComposeList( sFinSets,
                                       ...
                                       [ LiftAlongMonomorphism( sFinSets,
                                         ...
                                         InjectionOfCofactorOfCoproductWithGivenCoproduct( sFinSets,
                                                 B1,
                                                 IntGAP( 1 + AsList( pi313 )[1 + i] ),
                                                 N1 ) ],
                                       N1 ) )[1 + 0] ),
                  N1 );
    
    return PairGAP( Triple( N0, N1, N2 ),
                 @NTupleGAP( 8, id, s, t, is, it, ps, pt, mu ) );
...
@InstallMethod( NerveTruncatedInDegree2Data,
        [ IsCapCategory && IsFiniteCategory ],
...
@InstallMethod( YonedaNaturalEpimorphisms,
  ...
  function ( B )
    ...
    #% CAP_JIT_DROP_NEXT_STATEMENT
    @Assert( 0, HasIsFiniteCategory( B ) && IsFiniteCategory( B ) );
    
    #% CAP_JIT_DROP_NEXT_STATEMENT
    @Assert( 0, HasRangeCategoryOfHomomorphismStructure( B ) );
    ...
    N0 = PairGAP( ListWithIdenticalEntries( o, B0 ),
    ...
    return @NTupleGAP( 6, N0, N1, N2, pt, mu, s );
...
@InstallMethod( YonedaProjectionAsNaturalEpimorphism,
  ...
  function ( B )
    ...
    pt = NaturalTransformation(
                  N2,   ## The 2-Yoneda functor: B → sFinSets, c ↦ Hom(-, -) × Hom(-, c) && ψ ↦ Hom(-, -) × Hom(-, ψ)
                  Yepis[4],
...
@InstallMethod( TruthMorphismOfTrueToSieveFunctorAndEmbedding,
  ...
  function ( B )
    ...
    return @NTupleGAP( 5,
                   PairGAP( Sieves_objects,
                         Sieves_morphisms ),
                   PairGAP( ListWithIdenticalEntries( lobjs, D ),
                         ListWithIdenticalEntries( lmors, IdentityMorphism( sFinSets, D ) ) ),
                   PairGAP( HomHomOmega_objects,
